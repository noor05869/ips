...
class PostData {
    ...
    SubmitDocument = (data, func) => {
        console.log({ data })
        const res = async () => {

            let formData = new FormData()
            formData.append('uid', data.uid);
            formData.append('cnic_front', data.CNICFrontFile);
            formData.append('cnic_back', data.CNICBackFile);
            formData.append('MailingAddProof', data.MailingAddProof);
            formData.append('IncomeProof', data.IncomeProof);
            formData.append('kinCNIC_front', data.kinCNICFrontFile);
            formData.append('kinCNIC_back', data.kinCNICBackFile);
        ...
        const res = async () => {
            const resp = await axios
                ...
                .post("https://ips-multinet-staging.finqalab.com/v1/form/submit", {
                    ...
                    USTIN: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.USTIN : '',
                    USGREENCARD: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.USGREENCARDNum : '',
                    OverseasAddress: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.OverseasAddress : '',
                    frgnNumber: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.frgnNumber : '',
                    PWRAttorney: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.PWRAttorney : '',
                    AttorneyAddress: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.AttorneyAddress : '',
                    renouncedCitizenchip: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.renouncedCitizenchip : '',
                    taxformSubmit: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' ? data?.taxformSubmit : '',
                    formSubmitDate: data?.USCitizen !== 'NO' && data?.USGREENCARD !== 'NO' && data.taxformSubmit === '' ? data?.formSubmitDate : '',
